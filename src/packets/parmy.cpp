...
void parmy::process()
{
    ...
    if (command == "IsDropItemInCastle")
    {
        data2["ok"]=1;
        data2["packageId"]=0.0;
        obj2["cmd"]="army.IsDropItemInCastle";
        gserver.SendObject(client,obj2);
        return;
    }
    ...
    if (command == "newArmy")
    {
        ...
        if (missiontype == MISSION_ATTACK || missiontype == MISSION_SCOUT)// scout or attack
        {
            if (client->Beginner() && tile->m_type > 10)
            {
            ...
            if (tile->m_ownerid > 0)
            {
                ...
                if (oclient->Beginner() && tile->m_type==CASTLE)
                {
                    //attacking beginner
                ...
                if (missiontype==MISSION_SCOUT && troops.scout == 0) {
                    gserver.SendObject(client,gserver.CreateError("army.newArmy",-61,"Scouts are required for this mission."));
                    return;
                }
                //you have enough troops, valid target, gogogo
                Hero * hero = nullptr;
                if (heroid < 0) {
                    if (missiontype==MISSION_ATTACK) {
                        gserver.SendObject(client,gserver.CreateError("army.newArmy",-71,"You must appoint a hero to lead the attack."));
                        return;
                    }
                }
                else {
                ...
                am->direction = DIRECTION_FORWARD;
                ...
                am->reachtime = am->starttime + gserver.CalcTroopSpeed(city, troops, city->m_tileid, targettile) * 1000 + resttime;
                ...
                if (missiontype==MISSION_ATTACK) {
                    oclient->enemyarmymovement.push_back(am);
        ...
        else if (missiontype == MISSION_REINF)
        {
